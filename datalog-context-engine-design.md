...
The gap in the literature is well-defined: **no agent-memory system uses derived-relation maintenance** — i.e., an actual deductive engine where "memory" is base facts + rules, incrementally maintained. RelationalAI demonstrated that Datalog can be a full knowledge-graph management system (Rel, arXiv:2504.10323); nobody has applied that thesis to LLM context. The benchmarks that reward exactly this (temporal reasoning, knowledge updates, multi-hop) are the ones current systems fail.
